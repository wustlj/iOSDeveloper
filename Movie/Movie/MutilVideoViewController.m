...
- (void)viewDidLoad
{
    ...
    [btn4 setFrame:CGRectMake(150, 434, 120, 50)];
    [btn4 setBackgroundColor:[UIColor redColor]];
    [btn4 setTitle:@"Cancel" forState:UIControlStateNormal];
    [btn4 addTarget:self action:@selector(cancelAction) forControlEvents:UIControlEventTouchUpInside];
...
{
    if (!_baseMovie) {
        NSURL *videoURL = [NSURL fileURLWithPath:[[NSBundle mainBundle] pathForResource:@"move" ofType:@"MOV"]];
...
//        MovieComposition *c4 = [[MovieComposition alloc] initWithURL:videoURL4 timeRange:range4];
        ...
        _baseMovie = [[GPUMultiMovie alloc] initWithVideos:@[c1]];
    }
